#include <iostream>
#include <vector>
#include <string>
#include <stdexcept>
#include <optional>

class OperationRecorder {
private:
    size_t buffer_size;
    std::vector<std::optional<std::string>> buffer;
    size_t tail;
    size_t count;
    size_t undocount;
    size_t total;

    size_t next(size_t ptr, size_t step = 1) {
        return (ptr + step) % buffer_size;
    }

    size_t prev(size_t ptr, size_t step = 1) {
        return (ptr + buffer_size - step) % buffer_size;
    }
public:
    OperationRecorder(size_t size = 10000)
        : buffer_size(size), buffer(), tail(0), count(0), undocount(0), total(0) {}
    
    void record(const std::string& data) {
        if (undocount > 0) {
            count -= undocount;
            undocount = 0;
        }

        if (tail == total) {
            buffer.push_back(data);
            total++;
        } else {
            buffer[tail] = data;
        }

        tail = next(tail);
        if (count < buffer_size) {
            count++;
        }
    }

    std::optional<std::string> undo() {
        if (undocount >= count) {
            return std::nullopt;
        }

        tail = prev(tail);
        undocount++;
        return buffer[tail];
    }

    std::optional<std::string> redo() {
        if (undocount == 0) {
            return std::nullopt;
        }
        
        undocount--;
        std::string tmp = *buffer[tail];
        tail = next(tail);
        return tmp;
    }

    void clear() {
        tail = 0;
        count = 0;
        undocount = 0;
    }
};

int main() {
    OperationRecorder recorder(5);
    recorder.record("Operation 1");
    recorder.record("Operation 2");
    recorder.record("Operation 3");

    auto op = recorder.undo();
    if (op) std::cout << "Undid: " << *op << std::endl;

    op = recorder.redo();
    if (op) std::cout << "Redid: " << *op << std::endl;

    recorder.clear();
    return 0;
}